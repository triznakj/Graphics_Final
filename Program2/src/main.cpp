...
GLuint trapVertexArray;

GLuint trapPosBuffer;
GLuint trapColorBuffer;
GLuint animateTrapBuffer;

GLuint trapPosSlot;
GLuint trapColorSlot;
GLuint animateTrapSlot;
...
GLfloat trapezoid[8] = { -.75, -.5, -.6, -.6, -.4, -.6, -.25, -.5 };
GLfloat trapezoidColors[12] = { 1, 0, 0, 0, 1, 0, 0, 0, 1, 0, 0, 0 };

GLfloat animateTrap[4] = {.5, 2, 1, 3};
...
void display(WorldState & state)
{
	...
	// Use a simple shader to render the line
	glUseProgram(trapShaderProg);
	checkGLError("useProgram");

	//use the vertex array
	glBindVertexArray(trapVertexArray);

	//get uniform slot id
	//TODO: send a uniform value to the shader
	timeSlot = glGetUniformLocation(trapShaderProg, "timeVal");
	glUniform1f(timeSlot, state.getCurrentTime());
	checkGLError("uniform");

	//new uniform
	GLint mousePressed = glGetUniformLocation(trapShaderProg, "mousePressed");
	int temp = 0;
	if (sf::Mouse::isButtonPressed(sf::Mouse::Left))
	{
		temp = 1;
	}
	glUniform1i (mousePressed, temp);
	checkGLError("uniform");

	glBindBuffer(GL_ARRAY_BUFFER, trapPosBuffer);
	glEnableVertexAttribArray(trapPosSlot);
	glVertexAttribPointer(trapPosSlot, 2, GL_FLOAT, GL_FALSE, 0, 0);
	checkGLError("vertex attrib pos");

	// Do the same thing for colors
	glBindBuffer(GL_ARRAY_BUFFER, trapColorBuffer);
	glEnableVertexAttribArray(trapColorSlot);
	glVertexAttribPointer(trapColorSlot,3, GL_FLOAT, GL_FALSE, 0, 0);
	checkGLError("color array setup");
	
	// Should animate!
	glBindBuffer(GL_ARRAY_BUFFER, animateTrapBuffer);
	glEnableVertexAttribArray(animateTrapSlot);
	glVertexAttribPointer(animateTrapSlot, 1, GL_FLOAT, GL_FALSE, 0, 0);
	checkGLError("vertex attrib speed");
...
void graphicsInit()
{
	...
	//animateSlot = glGetAttribLocation(shaderProg, "shouldAnimate");
	...
	char const * trapVertPath = "shaders/simple.vert";
	char const * trapFragPath = "shaders/simple.frag";
	trapShaderProg = ShaderManager::shaderFromFile(&trapVertPath, &trapFragPath, 1, 1);

	trapPosSlot = glGetAttribLocation(trapShaderProg, "pos");
	trapColorSlot = glGetAttribLocation(trapShaderProg, "color");
	animateTrapSlot = glGetAttribLocation(trapShaderProg, "speed");
	if (trapPosSlot < 0 || trapColorSlot < 0 || animateTrapSlot < 0) {
		fprintf(stderr, "Could not get input location\n");
	}
	

	glGenBuffers(1, &trapPosBuffer); //make the position buffer
	glGenBuffers(1, &trapColorBuffer);
	glGenBuffers(1, &animateTrapBuffer);
	if (trapPosBuffer == 0) fprintf(stderr, "GenBuffer failed\n");
	if (trapColorBuffer == 0) fprintf(stderr, "GenBuffer failed\n");
	if (animateTrapBuffer == 0) fprintf(stderr, "GenBuffer failed\n");

	glGenVertexArrays(1, &trapVertexArray);
	glBindBuffer(GL_ARRAY_BUFFER, trapPosBuffer); //activate the position buffer
	glBufferData(GL_ARRAY_BUFFER, sizeof(trapezoid), trapezoid, GL_STATIC_DRAW); //alloc and upload
	glBindBuffer(GL_ARRAY_BUFFER, 0); //deactivate the position buffer
	checkGLError("trap pos buffer");

	// Do the same thing for the color data
	glBindBuffer(GL_ARRAY_BUFFER, trapColorBuffer);
	glBufferData(GL_ARRAY_BUFFER, sizeof(trapezoidColors), trapezoidColors, GL_STATIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
	checkGLError("trap color buffer");
	
	// Do the same thing for the speed data
	glBindBuffer(GL_ARRAY_BUFFER, animateTrapBuffer);
	glBufferData(GL_ARRAY_BUFFER, sizeof(animateTrap), animateTrap, GL_STATIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
	checkGLError("animate trap buffer");
		// END ADDED CODE *********************************************************************************************
}
