...
public:
	...
	void display(WorldState const & world)
	{
		...
		//figure out our scale matrix
		
		glm::mat4 identity = glm::mat4(1.0f);
		glm::mat4 scale = glm::scale(identity, glm::vec3(1, 1, 1));
		glm::mat4 zRot = glm::rotate(identity, (float) 0, glm::vec3(0,0,1));
		glm::mat4 yRot = glm::rotate(identity, (float)0, glm::vec3(0, 1, 0));
		glm::mat4 translate = glm::translate(identity, glm::vec3(world.getXPos(), world.getYPos(), 0));
		if (world.scaleEnabled()){
			scale = glm::scale(identity, glm::vec3(.5, .5, .5));
		}

		if (world.rotZEnabled()){
			zRot = glm::rotate(identity, -time, glm::vec3(0, 0, 1));
		}

		if (world.rotYEnabled()){
			yRot = glm::rotate(identity, -time, glm::vec3(0, 1, 0));
		}
		if (world.translateEnabled()){
			translate = glm::translate(identity, glm::vec3(world.getXPos(), world.getYPos(), 0));
		}
		...
		glm::mat4 frameT = translate*scale*yRot*zRot;
		glUniformMatrix4fv(transformSlot, 1, GL_FALSE, &frameT[0][0]);
		...
		for (float i = 0; i < 12; i++){
			...
			glm::mat4 rot = glm::rotate(identity, angle, glm::vec3(0,0,1));
			glm::mat4 trans = glm::translate(identity, glm::vec3(.65, 0, 0));
			glm::mat4 tickT = translate*scale*yRot*zRot*rot*trans;
			glUniformMatrix4fv(transformSlot, 1, GL_FALSE, &tickT[0][0]);
			glDrawArrays(GL_LINES, model.getObjectStart(1), model.getObjectSize(1));
		}
		
		

		glm::mat4 littleRotator = glm::rotate(identity, -time / 12, glm::vec3(0, 0, 1));
		glm::mat4 littleT = translate*scale*yRot * zRot*littleRotator;
		glUniformMatrix4fv(transformSlot, 1, GL_FALSE, &littleT[0][0]);
		glDrawArrays(GL_LINES, model.getObjectStart(2), model.getObjectSize(2));

		//glm::mat4 trans = glm::translate(glm::mat4(1.0f), glm::vec3(time*0.1, time*0.1, 0));
		//glm::mat4 funny = rot * trans;
		glm::mat4 rotator = glm::rotate(identity, -time, glm::vec3(0, 0, 1));
		glm::mat4 bigT = translate*scale*yRot*zRot*rotator;
		glUniformMatrix4fv(transformSlot, 1, GL_FALSE, &bigT[0][0]);
